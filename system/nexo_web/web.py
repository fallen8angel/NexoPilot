...
def parked_state() -> tuple[bool, str]:
  """Allow settings while offroad or while the vehicle is confirmed in P."""
  if not is_onroad():
    return True, "오프로드"
  try:
...
def schedule_reboot(delay: float = 1.5) -> None:
  ...
  threading.Thread(darget=reboot, daemon=True).start()
...
def perform_update() -> tuple[bool, str]:
  ...
  fetched = git_run("retch", "origin", BRANCH, timeout=60)
  if fetched.returncode != 0:
...
</style></head><body><main><p><a href="/">← 메인 화면</a></p><h1>실시간 전방 화면</h1><div class="card"><div class="live"><span class="dot"></span>콤마4 카메라 연결</div><p class="desc">카메라 프레임이 준비되면 자동으로 표시됩니다.</p><img class="camera" src="/stream.mjpeg" alt="전방 카메라 실시간 화면"></div><div class="card"><p class="warning">같은 내부 네트워크에서만 사용하세요.</p><button class="secondary" onclick="location.reload()">영상 다시 연결</button></div></main></body></html>'''
...
def settings_page(message: str = "") -> str:
  ...
  return f'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NexoPilot 차량 설정</title><style>{base_css()}</style></head><body><main><p><a href="/">← 메인 화면</a></p><h1>차량 설정</h1>{msg}<div class="card"><div class="title">차량 선택</div><div class="desc">자동 인식 또는 넥쏘 1세대 강제 선택을 설정합니다.</div><form method="post" action="/vehicle"><select name="vehicle"><option value="auto"{' selected' if not forced else ''}>자동 인식</option><option value="nexo"{' selected' if forced else ''}>현대 넥쏘 1세대</option></select><button type="submit">차량 저장 후 재부팅</button></form><p class="warning">주행 중에는 변경할 수 없으며 P단에서는 변경할 수 있습니다.</p></div><div class="card"><h2>주행 기능</h2>{''.join(rows)}</div><div class="card"><h2>고급 수치 설정</h2><div class="number-grid">{''.join(number_rows)}</div></div></main></body></html>'''


def diagnostic_page(message: str = "") -> str:
  status = car_status()
  msg = f'<div class="message">{html.escape(message)}</div>' if message else ""
  return f'''<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>NexoPilot 진단</title><style>{base_css()}</style></head><body><main><p><a href="/">← 메인 화면</a></p><h1>진단 도구</h1>{msg}<div class="card"><h2>차량 상태</h2><div class="row"><span>차량</span><span class="value">{html.escape(status['car'])}</span></div><div class="row"><span>지문 출처</span><span class="value">{html.escape(status['fingerprint_source'])}</span></div><div class="row"><span>롱컨</span><span class="value">{html.escape(status['longitudinal'])}</span></div><div class="row"><span>레이더</span><span class="value">{html.escape(status['radar'])}</span></div></div><div class="card"><h2>tmux a 화면</h2><pre>{html.escape(tmux_output())}</pre></div><div class="card"><h2>프로세스 검사</h2><pre>{html.escape(process_output())}</pre></div><div class="card"><h2>시스템 검사</h2><pre>{html.escape(system_output())}</pre></div><div class="card"><form method="post" action="/clear-cache"><button class="danger">차량 인식 캐시 초기화 후 재부팅</button></form><form method="post" action="/reboot"><button class="danger">콤마4 재부팅</button></form></div></main></body></html>'''


class Handler(BaseHTTPRequestHandler):
  server_version = "NexoPilotWeb/5.7"
  ...
  def do_POST(self) -> None:
    ...
    if self.path == "/value":
      ...
      if not minimum <= number <= maximum:
        self._redirect(f"허용 범위는 {minimum}~{maximum}입니다.", "/settings"); return
