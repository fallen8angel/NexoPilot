...
def diagnostic_page(message: str = "") -> str:
  ...
  <div class="card"><div class="title">8초 통합진단 파일</div><div class="desc">제어 스택을 최대 10초 기다린 뒤 차량 인식·운전자 감시·레이더·SCC/FCA·주차센서·순정 내비·Panda 안전 상태를 8초간 수집합니다. 실시간 메시지가 전부 0건이면 차량 고장이 아닌 ‘진단 불가’로 표시합니다.</div><form method="post" action="/diagnostics/capture"><button>8초 통합진단 파일 하나 받기</button></form><form method="post" action="/diagnostics/download-last"><button class="secondary">방금 진단 파일 다시 다운받기</button></form><div class="desc">다시 다운받기는 새로 수집하지 않고 방금 완료된 동일한 파일을 내려받습니다.</div></div>
